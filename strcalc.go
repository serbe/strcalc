...
func findMul(src string) string {
	length := len(src)
	if length < 3 {
		return src
	}
...
func getNearNumbers(src string, pos int) string {
	findLeft := false
	findRight := false
	leftValue := ""
	rightValue := ""
	for i := 1; i < pos; i++ {
		...
		if !findRight {
			if isNumeric(src[pos+i]) {
				leftValue = leftValue + string(src[pos+i])
			} else {
				findRight = true
			}
		}
	...
	return leftValue + string(src[pos]) + rightValue
}

func getTwoNumber(src string, pos int) (string, string) {
	findLeft := false
	findRight := false
	leftValue := ""
	rightValue := ""
...
func calc2(src string) string {
	f := brackets(src)
	if f != src {
		src = strings.Replace(src, "("+f+")", calc2(f), 1)
		src = calc2(src)
	}

	f = findMul(src)
	if f != "" {
		src = strings.Replace(src, f, mul(f), 1)
		src = calc2(src)
	}
	reAdd := regexp.MustCompile(`(\d+\+\d+)`)
	reSub := regexp.MustCompile(`(\d+\-\d+)`)
	if reAdd.MatchString(src) && reSub.MatchString(src) {
		if reAdd.FindStringSubmatchIndex(src)[0] < reSub.FindStringSubmatchIndex(src)[0] {
			f := reAdd.FindStringSubmatch(src)[1]
			src = strings.Replace(src, f, add(f), 1)
			src = calc2(src)
		} else {
			f := reSub.FindStringSubmatch(src)[1]
			src = strings.Replace(src, f, sub(f), 1)
			src = calc2(src)
		}
	}
	if reAdd.MatchString(src) {
		f := reAdd.FindStringSubmatch(src)[1]
		src = strings.Replace(src, f, add(f), 1)
		src = calc2(src)
	}
	// reSub := regexp.MustCompile(`(\d+\-\d+)`)
	if reSub.MatchString(src) {
		f := reSub.FindStringSubmatch(src)[1]
		src = strings.Replace(src, f, sub(f), 1)
		src = calc2(src)
	}
	reRem := regexp.MustCompile(`(\d+\%\d+)`)
	if reRem.MatchString(src) {
		f := reRem.FindStringSubmatch(src)[1]
		src = strings.Replace(src, f, rem(f), 1)
		src = calc2(src)
	}
	return src
}
...
func calc(src string) string {
	...
	if reBrackets.MatchString(src) {
		f := reBrackets.FindStringSubmatch(src)[1]
		src = strings.Replace(src, "("+f+")", calc(f), 1)
		src = calc(src)
	}
	reMul := regexp.MustCompile(`(\d+\*\d+)`)
	if reMul.MatchString(src) {
		f := reMul.FindStringSubmatch(src)[1]
		src = strings.Replace(src, f, mul(f), 1)
		src = calc(src)
	}
	reAdd := regexp.MustCompile(`(\d+\+\d+)`)
